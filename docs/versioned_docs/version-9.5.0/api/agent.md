...
Agent calls are automatically traced to `~/.underthesea/traces/` as JSON files. Disable with `UNDERTHESEA_TRACE_DISABLED=1`.
...
# Auto-trace (default) — traces saved to ~/.underthesea/traces/
bot = Agent("my-bot", tools=[calculator_tool])
bot("What is 2+2?")
# Console output shows trace with timing, tokens, tool calls

# Custom trace directory
tracer = LocalTracer(trace_dir="/tmp/my-traces")
bot = Agent("my-bot", tools=[calculator_tool], tracer=tracer)
bot("What is 2+2?")

# Browse traces
tracer.list_traces(limit=10)
...
Wrap functions with `@trace` to create traces. Nested decorated functions become child spans automatically.
...
@trace(tracer)
def pipeline(text):
    cleaned = preprocess(text)       # becomes a child span
    bot = Agent("bot", tools=[calculator_tool])
    return bot(cleaned)              # agent spans also nest automatically

pipeline("  Calculate 2+2  ")
# Creates 1 trace with nested spans: pipeline → preprocess, llm.chat, tool, llm.chat
```
...
|----------|-------------|---------|
| `UNDERTHESEA_TRACE_DISABLED` | Disable all tracing (`1`, `true`, `yes`) | Not set (tracing enabled) |
| `UNDERTHESEA_TRACE_DIR` | Custom directory for auto-trace files | `~/.underthesea/traces` |
