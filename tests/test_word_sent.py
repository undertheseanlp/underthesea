...
class TestWord_sent(TestCase):
    ...
    def test_word_sent_4(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Chúng_ta thường nói đến Rau_sạch , Rau an_toàn để phân_biệt với các rau bình_thường bán ngoài chợ .'
        self.assertEqual(actual_sentence, expected_sentence)
    ...
    def test_word_sent_7(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Có_lẽ không ở đâu trên khắp thế_giới bóng_đá có giải vô_địch_quốc_gia chịu_chơi , chịu chi như giải nhà_nghề Trung_Quốc .'
        self.assertEqual(actual_sentence, expected_sentence)
    ...
    def test_word_sent_8(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Số những ngôi_sao về chiều gia_nhập giải nhà_nghề Trung_Quốc có Carlos_Tevez ( Argentina ) , Graziano_Pelle ( Italia ) , Obafemi_Martins ( Nigeria ) .'
        self.assertEqual(actual_sentence, expected_sentence)
    ...
    def test_word_sent_9(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Liên_quan đến sự_việc trên , sáng 30 / 3 , trao_đổi với phóng_viên Dân_trí , Thiếu_tá Nguyễn_Anh_Dũng - Phó_trưởng Công_an TP Vĩnh_Yên ( Vĩnh_Phúc ) - cho_biết , ngay khi nhận được thông_tin , đơn_vị này đã vào cuộc điều_tra và xác_định người đánh_đập cháu H . là ông Hoàng_Văn_L . ( bố_đẻ của cháu H . ) .'
        self.assertEqual(actual_sentence, expected_sentence)
    ...
    def test_word_sent_10(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Phát_biểu trước báo_giới , cựu tiền_vệ MU , Paul_Scholes đánh_giá MU đứng trước cơ_hội lớn vô_địch Europa_League'
        self.assertEqual(actual_sentence, expected_sentence)
    ...
    def test_word_sent_12(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Tích_phân là một khái_niệm toán_học , và cùng_với nghịch_đảo của nó vi_phân ( differentiation ) đóng vai_trò là 2 phép_tính cơ_bản và chủ_chốt trong lĩnh_vực giải_tích ( calculus ) .'
        self.assertEqual(actual_sentence, expected_sentence)

    def test_word_sent_13(self):
        sentence = u"Ý tưởng chủ đạo là tích phân và vi phân là hai phép tính nghịch đảo của nhau . "
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Ý_tưởng chủ_đạo là tích_phân và vi_phân là hai phép_tính nghịch_đảo của nhau .'
        self.assertEqual(actual_sentence, expected_sentence)

    def test_word_sent_14(self):
        sentence = u"Vì ngày lang thang vẫn còn dài nên ta không kéo ga thật sâu"
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Vì ngày lang_thang vẫn còn dài nên ta không kéo ga thật sâu'
        self.assertEqual(actual_sentence, expected_sentence)
    ...
    def test_word_sent_15(self):
        ...
        actual_sentence = word_sent(sentence)
        expected_sentence = u'Vì_vậy , để hiểu thêm về ngôn_ngữ của người dân xứ Quảng qua đó hiểu về con_người vùng_đất này hơn và cũng để bổ_sung thêm nguồn tư_liệu dạy và học Ngữ_văn địa_phương , bài viết xin giới_thiệu một_số từ địa_phương so với từ_phổ_thông ( toàn_dân ) và một_số âm địa_phương so với âm_phổ_thông ( chuẩn - toàn_dân ) của người Quảng_Nam để các bạn đồng_nghiệp và các em học_sinh tham_khảo'
        self.assertEqual(actual_sentence, expected_sentence)
